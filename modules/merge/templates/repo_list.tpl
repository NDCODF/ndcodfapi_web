...
$(document).ready(function(){
    ...
    */
	...
	var ctype = $(that).attr('ctype');
	    ...
	    timeout: 5000,
	    error: function(data){
		alert($(that).attr('docname') + '<{$smarty.const._REPO_LIST_FAIL_LIST}>');
	    },
            success: function(data){
...
<{if isset($smarty.get.grpid) && $smarty.get.grpid > 0}>
    <{$smarty.const._REPO_LIST_TPL_TITLE_DEPARTMENT}>
<{else}>
    <{$smarty.const._REPO_LIST_TPL_TITLE_ALL}>
<{/if}>
<span class="badge"><{$total}></span>
</h3>
<h5>
<{$smarty.const._REPO_LIST_TPL_DEPARTMENT}>：
<select class="form-control_dummy" name="grpid">
<{html_options options=$grpopt selected=$smarty.get.grpid}>
</select>
<!-- *************************************** Start of groupby -->
<{$smarty.const._REPO_LIST_TPL_ORDER_BY}>：
...
<div class="row" style="background: <{cycle values="#F4F4F4, white"}>; padding-top: 8px;">
	...
	<{/if}>
	<{if $data.extname == 'otp'}>
	    <img src="images/LO_Impress.png"
                 alt="Impress 報表" />
	<{/if}>
	<h6>
	    <{$smarty.const._REPO_LIST_UPDATETIMES}>: <{$data.upcount}><br />
	    <{$smarty.const._REPO_LIST_LAST_UPDATE}><br />
	...
	<h5 class="textcolor">
		...
		<a class="commonlink downfile" title="<{$smarty.const._REPO_LIST_DOWNLOAD_TEMP}>"
                   href="repo_file.php?id=<{$data.rec_id}>">
			<span class="glyphicon glyphicon-download-alt">
			</span>
		        <{$smarty.const._REPO_LIST_DOWNLOAD_TEMP}>
		</a>
	    </span>&nbsp;
	    <{if !$data.no_perm || ($data.no_perm && $data.for_admin)}>
	    <span class="text-success">
		<a class="commonlink modify" title="<{$smarty.const._REPO_LIST_MODIFY}>" href=""
		   rec_id="<{$data.rec_id}>" cid="<{$data.cid}>">
			<span class="glyphicon glyphicon-pencil"></span>
                        <{$smarty.const._REPO_LIST_MODIFY}>
		</a>
	    </span>&nbsp;
	    <span style="font-color: black; a:color: black;">
		<a class="deletelink" title="<{$smarty.const._REPO_LIST_DELETE}>"
		   href="<{$acturi}>&id=<{$data.rec_id}>&sta=4">
			<span class="glyphicon glyphicon-trash"></span>
		        <{$smarty.const._REPO_LIST_DELETE}>
		</a>
	    </span>&nbsp;
	    <{/if}>
	    <{*if $data.for_admin*}>
		...
		<a class="otherlink assign-perm" href=""
		   ...
		   rec_id="<{$data.rec_id}>"
		   title="<{$smarty.const._REPO_LIST_SHARE}>">
		        <span class="glyphicon glyphicon-share"></span>
			<{$smarty.const._REPO_LIST_SHARE}>
		</a>
	    </span>&nbsp;
	    <{/if}>
	    <{if !$data.no_perm}>
	    <span class="text-success">
		<a class="otherlink" title="<{$smarty.const._REPO_LIST_CHECK_MESSAGE}>"
		   href="<{$origuri}>&wo=<{$wo}>&id=<{$data.rec_id}>&start=<{$start}>"
		   >
		        <span class="otherlink glyphicon glyphicon-eye-open">
			</span>
			<{$smarty.const._REPO_LIST_CHECK_MESSAGE}>
		</a>
	    </span>
	    <{/if}>
	    <{if $data.no_perm || ($data.no_perm && $data.for_admin)}>
	    <span class="text-success">
		<a class="otherlink" title="<{$smarty.const._REPO_LIST_LEAVE_MESSAGE}>"
		   href="<{$origuri}>&wo=<{$wo}>&id=<{$data.rec_id}>"
		   >
                        <span class="glyphicon glyphicon-eye-open"></span>
			<{$smarty.const._REPO_LIST_LEAVE_MESSAGE}>
		</a>
	    </span>&nbsp;
	    <span>
		<span class="glyphicon glyphicon-user"></span>
                <{if $data.for_admin}>
		<{$data.uname}><{$smarty.const._REPO_LIST_CREATE_BY}>
		<{else}>
                <{$data.uname}><{$smarty.const._REPO_LIST_CREATE_BY}>
		<{/if}>
	    </span>
...
<{includeq file="db:repo_dialogs.tpl"}>
