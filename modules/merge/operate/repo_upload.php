...
function uploadFile(&$db, &$cnf, $tbl, $id)
{
    ...
    $copyto = "{$cnf['templaterepodir']}/{$uuid}.{$ext}";
...
function checkDocname($docname, $cid, $extname)
{
    ...
    $sql_docname_same = "SELECT Count(docname) from ndcodfapi.ndc_repo_user_templates where docname = '{$docname}' and cid = {$cid} and extname = '{$extname}'";
    ...
    $sql_docname_autonew = "SELECT Count(docname) from ndcodfapi.ndc_repo_user_templates where docname like '{$docname}_(%)' and cid = {$cid} and extname = '{$extname}'";
...
function getCIDList(){
    global $xoopsDB;
    $sql_title = "SELECT * from ndcodfapi.ndc_repo_category";
...
$allCID = getCIDList();
file_put_contents('/usr/share/NDCODFAPI/ODFReport/templates/data.json', json_encode($allCID));
...
if (isset($_POST['wo']) && 'upload' == $_POST['wo'])
{
    if (trim($_POST['id']) == '')  // 上傳新的
    {
        ...
        if (!empty($uuid))
        {
            ...
            redirect_header('index.php?op=operate&subop=repo_list',
                            2, 'OK');
            exit();
        }
        else
    ...
    else  // 修改現有紀錄, 若有選擇上傳檔案則也上傳檔案
    {
        ...
        if (0 == $_FILES['uploadFile']['error'])
        {
            ...
            if ($oldext != $ext)
                @unlink("{$cnf['templaterepodir']}/{$uuid}.{$oldext}");
        ...
        generateTemplateInfo2JSON();
        redirect_header('index.php?op=operate&subop=repo_list', 2, 'OK');
        exit();
    }
}
else
    $xoopsOption['template_main'] = 'repo_main.tpl';
?>
