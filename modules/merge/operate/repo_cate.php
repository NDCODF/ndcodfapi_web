...
global $xoopsDB;
$db = $xoopsDB;
$categoryTb = $xoopsDB->prefix('repo_category');
...
function addNewCID(){
    ...
    $sql_title_same = "SELECT Count(title) from ndcodfapi.ndc_repo_category where title = '{$title}'";
    $rs_docname  = $xoopsDB->queryF($sql_title_same);
    $obj = 0;
    ...
    $cate->set("cid",$cid);
    $cate->update();
    return array('status' => true, 'ret' => "OK");
}
...
function updateCID(){
    ...
    $sql_update_title = "Update ndcodfapi.ndc_repo_category set title = '{$title}' where cid = {$cid}";
    $xoopsDB->queryF($sql_update_title);
    return array('status' => true, 'ret' => "OK");
}
function getCIDList(){
    global $xoopsDB;
...
$nav = new MergePageNav($abilists['total'], $limit,
                        $start, 'start', 'op=operate&subop=repo_cate');
$tpl->assign('navPath', $nav->renderNav());

$xoopsOption['template_main'] = 'repo_cate.tpl';
?>
